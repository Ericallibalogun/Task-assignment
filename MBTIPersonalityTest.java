...
public class MBTIPersonalityTest{
	...
	Scanner userInput = new Scanner(System.in);
		...
		String  nameResponse = userInput.next();
		...
		for (int loop = 0; loop < choices.length; loop++) {
            		System.out.println(choices[loop]);
            		String answer = userInput.next().toUpperCase();

		if (answer.equals("A")) {
		countA++;
                switch (categories[loop]) {
		...
		String[][] traits = {
			...
			System.out.println("---------------------------");
       			System.out.println("Hello, " + nameResponse + "!");
